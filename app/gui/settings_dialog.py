...
class SettingsDialog(QDialog):
    def __init__(self, cfg: AppConfig, parent=None):
        ...
        # --- Движок -----------------------------------------------------
        g_engine = QGroupBox("Движок транскрибации")
        v = QVBoxLayout(g_engine)

        self.rb_local = QRadioButton("Локальный Whisper — офлайн и бесплатно (faster-whisper)")
        self.rb_cloud = QRadioButton("Облачный API — быстро, нужен API-ключ")
        ...
        self.cmb_device.addItem("Авто (GPU, если доступен)", "auto")
        self.cmb_device.addItem("CPU", "cpu")
        self.cmb_device.addItem("GPU (NVIDIA CUDA)", "cuda")
        self.chk_vad = QCheckBox("Фильтр тишины (VAD) — убирает галлюцинации на паузах")
        local_form.addRow("Модель:", self.cmb_model)
        local_form.addRow("Устройство:", self.cmb_device)
        ...
        self.cmb_provider.addItem("Groq — whisper-large-v3, быстрый и дешёвый", "groq")
        self.cmb_provider.addItem("OpenAI — whisper-1", "openai")
        self.ed_groq = QLineEdit()
        self.ed_openai = QLineEdit()
        keys_hint = QLabel(
            'Ключи: <a href="https://console.groq.com/keys">console.groq.com/keys</a> · '
            '<a href="https://platform.openai.com/api-keys">platform.openai.com/api-keys</a>'
        )
        keys_hint.setOpenExternalLinks(True)
        cloud_form.addRow("Провайдер:", self.cmb_provider)
        cloud_form.addRow("Ключ Groq:", _key_row(self.ed_groq))
        cloud_form.addRow("Ключ OpenAI:", _key_row(self.ed_openai))
        cloud_form.addRow("", keys_hint)
        v.addLayout(cloud_form)
        root.addWidget(g_engine)

        # --- Диаризация ---------------------------------------------------
        g_diar = QGroupBox("Определение спикеров (диаризация)")
        f = QFormLayout(g_diar)
        self.ed_hf = QLineEdit()
        f.addRow("Токен Hugging Face:", _key_row(self.ed_hf))
        status = "установлены" if diarize.is_available() else \
            "не установлены — выполните install-diarization.bat"
        diar_hint = QLabel(
            f"Пакеты pyannote/torch: <b>{status}</b>.<br>"
            'Токен: <a href="https://huggingface.co/settings/tokens">huggingface.co/settings/tokens</a> '
            "(бесплатно). Затем примите условия моделей:<br>"
            '<a href="https://huggingface.co/pyannote/speaker-diarization-3.1">speaker-diarization-3.1</a> и '
            '<a href="https://huggingface.co/pyannote/segmentation-3.0">segmentation-3.0</a>'
        )
        ...
        f2.addRow("Язык видео по умолчанию:", self.cmb_lang)
        f2.addRow("Папка сохранения:", row)
        root.addWidget(g_common)

        note = QLabel("Ключи хранятся локально в файле настроек на этом компьютере.")
